...
local function BuildOptionsPanel()
    ...
    langLabel:SetPoint("TOPLEFT", versionFS, "BOTTOMLEFT", 0, -18)
    ...
    UIDropDownMenu_SetWidth(langDD, 200)
    ...
    langResetBtn:SetSize(160, 26)
    langResetBtn:SetPoint("LEFT", langDD, "RIGHT", -4, 2)
    langResetBtn:SetText(RG_L["OPT_RESET_BTN"] or "Reset to defaults")
    ...
    listDD:SetPoint("TOPLEFT", listLabel, "BOTTOMLEFT", -15, -4)
    ...
    ---------- OnShow: sync UI state every time the panel becomes visible ----------
    panel:SetScript("OnShow", function()
        local langVal = RandomGreetingDB.locale or "auto"
        UIDropDownMenu_SetText(langDD, GetLangText(langVal))
        UIDropDownMenu_SetText(listDD, GetListLabel(selectedListKey))
        listScrollFrame.offset = 0
        listScrollFrame:SetVerticalScroll(0)
        RefreshListRows()
